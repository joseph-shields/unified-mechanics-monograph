...
pts = []
with open("../data/SPARC/MassModels_Lelli2016c.mrt") as f:
    lines = f.readlines()
dash = [i for i, l in enumerate(lines) if set(l.strip()) == {"-"}]
for l in lines[dash[-1]+1:]:
    if len(l.strip()) == 0: continue
...
for ID, R, Vobs, eV, Vgas, Vdisk, Vbul in pts:
    ...
    w.append(1.0 / max(2*eV/Vobs/math.log(10), 1e-3)**2)  # weight by log-error
    used_gal.add(ID)
...
json.dump(results, open("sparc_results.json", "w"), indent=2)
...
ax.set_title("Residuals — 2693-point SPARC sample", color=INK)
ax.legend(fontsize=9)
plt.tight_layout()
plt.savefig("sparc_rar.png", facecolor=BG)
